...
`ds-surrogate` is MIT-licensed (see [LICENSE](LICENSE)). The build pulls in
...
| nanopb 0.4.9 | zlib | Source files compiled into `ds_surrogate_core` from the `wpiutil/src/main/native/thirdparty/nanopb/` subtree of the allwpilib submodule. |
| cpp-httplib 0.18.5 | MIT | Fetched at build time via CMake `FetchContent` from yhirose/cpp-httplib. |
| GoogleTest 1.15.2 | BSD-3-Clause | Test-only dependency, fetched at build time via CMake `FetchContent` when `DS_SURROGATE_TESTS=ON`. |
